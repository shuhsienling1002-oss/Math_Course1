...
class GameEngine:
    ...
    def _end_game(self, status):
        ...
        if status == 'won':
            st.session_state.msg = "🎉 挑戰成功！答案正確！"
        else:
            st.session_state.msg = "❌ 挑戰失敗，請看下方計算過程。"
    ...
    def _generate_log(self):
        ...
        final_class = "color: #a6e3a1;" if self.current == self.target else "color: #f38ba8;"
        html += f"<div class='final-result' style='{final_class}'>🚩 最終結果：{self.current} (目標：{self.target})</div>"
        html += "</div>"
...
fill_class = "progress-fill warning" if float(curr) > float(tgt) else "progress-fill"

st.markdown(f"""
<div class="game-container">
    <div style="display: flex; justify-content: space-between; font-family: monospace;">
        <span>🏁 起點: {engine.start_val}</span>
        <span>🚩 目標: {tgt}</span>
    </div>
    <div class="progress-track">
        <div class="target-marker" style="left: {tgt_pct}%;"></div>
        <div class="{fill_class}" style="width: {curr_pct}%;"></div>
    </div>
    <div style="text-align: center; font-size: 24px; font-weight: bold;">
        當前數值: <span style="color: #89b4fa;">{curr}</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
if st.session_state.game_state == 'playing':
    ...
    with c2:
        if st.session_state.game_state == 'won':
            ...
            if st.button("🔄 重試 (Retry)", type="secondary", use_container_width=True):
                engine.retry()
                st.rerun()
